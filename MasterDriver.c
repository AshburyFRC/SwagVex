...
int getTicks(int distance) { // distance in inches
	...
	float ticksPerRotation = 627.2; //627.2 for high torque, 392 for high speed,
...
task autonomous {
	while (nMotorEncoder[backRightMotor] < getTicks(98)) {
		// do stuff

		ticksLeft = nMotorEncoder[backLeftMotor];
		ticksRight = nMotorEncoder[backRightMotor];
		...
		shootingMotorSpeed = 110;
		startTask(setShooting);
		wait1Msec(5000);
		motor[beltMotor1] = 100;
		motor[beltMotor2] = 100;
		wait1Msec(3000);
